...
public class FoodService implements FoodServiceImp {
    ...
    @Override
    public boolean updateFood(String newFileUpdate, int id, FoodDTO food) {



               if(food != null){
                   Food f = foodRepository.getById(id);
                   if(f != null){


                       f.setTitle(food.getTitle());
                       f.setTimeShip(food.getTitle());
                       f.setFreeShip(food.isFreeShip());
                       f.setPrice(food.getPrice());

                       Category category = new Category();
                       category.setId(food.getCateID());
